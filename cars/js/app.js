(() => {
    ...
    (() => {
        ...
        function rangeInit() {
            const priceSlider = document.getElementById("form-car-selection__range");
            const calculatorSlider = document.getElementById("form-calculator__range");
            if (priceSlider) {
                ...
                priceSlider.noUiSlider.on("update", (() => {
                    let currentValue = priceSlider.noUiSlider.get();
                    const tooltip = document.querySelector(".form-car-selection__range .noUi-tooltip");
                    const intervals = document.querySelectorAll(".form-car-selection__range .noUi-marker-large");
                    const interval15 = intervals[1];
                    const interval20 = intervals[2];
                    if (currentValue > 15e3) interval15.style.background = "#dd1a1e"; else interval15.style.background = "#ebebeb";
                    if (currentValue > 2e4) interval20.style.background = "#dd1a1e"; else interval20.style.background = "#ebebeb";
                    if (9999 == currentValue || 30001 == currentValue) tooltip.style.display = "none"; else tooltip.style.display = "block";
                }));
                const firstValue = document.querySelector(".form-car-selection__range .noUi-value-horizontal");
                const lastValue = document.querySelector(".form-car-selection__range .noUi-value-horizontal:last-child");
                firstValue.innerHTML = "< 10000";
                lastValue.innerHTML = "30000+";
            }
            if (calculatorSlider) {
                ...
                const firstValue = document.querySelector(".form-calculator__range .noUi-value-horizontal");
                const lastValue = document.querySelector(".form-calculator__range .noUi-value-horizontal:last-child");
                firstValue.innerHTML = "0 $";
                lastValue.innerHTML = "10000 $";
                const inputSlider = document.querySelector("#form-calculator__cost");
                calculatorSlider.noUiSlider.on("update", (() => {
                    let currentValue = calculatorSlider.noUiSlider.get();
                    inputSlider.value = currentValue;
                }));
                inputSlider.addEventListener("input", (() => {
                    calculatorSlider.noUiSlider.set(inputSlider.value);
                }));
            }
        }
        rangeInit();
        ...
        function initSliders() {
            let thumbsSliderContacts;
            if (document.querySelector(".thumbs-slider-contacts")) thumbsSliderContacts = new core(".thumbs-slider-contacts", {
                modules: [],
                observer: true,
                observeParents: true,
                slidesPerView: 3,
            ...
            if (document.querySelector(".main-slider-contacts")) new core(".main-slider-contacts", {
        ...
        da.init();
        document.addEventListener("click", headerSpollers);
        function headerSpollers(e) {
            const spollersMenu = document.querySelector(".spollers-menu");
            if (e.target.closest(".spollers-menu")) spollersMenu.classList.toggle("_active"); else if (spollersMenu.classList.contains("_active")) spollersMenu.classList.remove("_active");
        }
        if (document.getElementById("form-calculator__engine")) {
            const inputEngine = document.getElementById("form-calculator__engine");
            inputEngine.addEventListener("keydown", (event => {
                if (46 == event.keyCode || 8 == event.keyCode || 9 == event.keyCode || 27 == event.keyCode || 65 == event.keyCode && true === event.ctrlKey || event.keyCode >= 35 && event.keyCode <= 39) return; else if ((event.keyCode < 48 || event.keyCode > 57) && (event.keyCode < 96 || event.keyCode > 105)) event.preventDefault();
            }));
        }
        const script_button = document.querySelector(".form-car-selection__button");
        console.log(script_button);
        script_button.addEventListener("click", (e => {
            let form;
            form = document.querySelector(".form-car-selection");
            const fueltypes = {
                electric: "Электро",
                hybrid: "Гибрид",
                gasoline: "Бензин",
            ...
            const fueltype = fueltypes[form["fueltype"].value];
            const range = document.getElementById("form-car-selection__range");
            let rangeValue = range.noUiSlider.get();
            let rangeText;
            if (9999 == rangeValue) rangeText = "< 10000"; else if (30001 == rangeValue) rangeText = "30000+"; else rangeText = rangeValue;
            const carcases = {
                sedan: "Седан",
                hatchback: "Хэтчбек",
                coupe: "Купе",
            ...
            const carcase = carcases[form["carcase"].value];
            document.getElementById("your-choise-popup__item-value_fueltype").innerHTML = fueltype;
            document.getElementById("your-choise-popup__item-value_cost").innerHTML = rangeText;
            document.getElementById("your-choise-popup__item-value_carcase").innerHTML = carcase;
            flsModules.popup.open("#popup__car-selection");
            e.preventDefault();
        }));
        window["FLS"] = false;
        isWebp();
